...
Atmos retains the verified encrypted archive and restore evidence as a portfolio artifact. There is no recurring backup schedule, R2 lifecycle policy, or automatic backup deletion in the current CV showcase profile. Any retention configuration is future production hardening and requires a fresh owner approval because it deletes recovery artifacts.
